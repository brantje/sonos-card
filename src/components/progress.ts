...
class SonosPlayerProgress extends LitElement {
  _player: any;
  seekProgress: any;
  ...
  get width() {
    return this.shadowRoot?.querySelector<HTMLElement>('.sp-progress')?.offsetWidth;
  }
  ...
  render() {
    return html`
      <div class='sp-progress' @touchstart=${this.initSeek} @touchend=${this.handleSeek} @mousedown=${this.initSeek}
        @mouseup=${this.handleSeek} @mouseleave=${this.resetSeek} @click=${e => e.stopPropagation()}
        ?paused=${!this.player.isPlaying}>
        ${this.showTime ? html`
        <div class='sp-progress-duration'>
          <span>${convertProgress(this.seekProgress || this.progress)}</span>
          <div>
            ${this.showTime ? html`
            <span class='sp-progress-duration-remaining'>
        ...
        ` : ''}
        <paper-progress class=${classMap({
            transiting: !this.seekProgress,
            seeking: this.seekProgress ? this.seekProgress : '',
        })
      } .value=${this.seekProgress || this.progress} .max=${this.duration}>
        </paper-progress>
      </div>
    `;
  }
  ...
  initSeek(e) {
    ...
    this.seekWidth = this.width ? this.width : 0;
  ...
  static get styles() {
    return css`
      ...
      .sp-progress {
        --sp-player-height: 5px;
        --sp-player-progress-height: 5px;
        cursor: pointer;
        left: 0; right: 0; bottom: 0;
        position: absolute;
        pointer-events: auto;
        min-height: var(--sp-player-height);
        z-index: 3;

      }
      .sp-progress-duration {
        left: calc(var(--ha-card-border-radius, 4px) / 2);
        right: calc(var(--ha-card-border-radius, 4px) / 2);
        bottom: calc(var(--sp-player-height) + 6px);
        position: absolute;
        display: flex;
      ...
      .sp-progress-duration-remaining {
        opacity: .5;
      }
      paper-progress {
        height: var(--sp-player-progress-height);
        --paper-progress-height: var(--sp-player-progress-height);
        bottom: 0;
        position: absolute;
        width: 100%;
        transition: height 0;
        ...
        --paper-progress-container-color: rgba(0,0,0,0.9);
      }
      paper-progress.seeking {
        transition: height .15s ease-out;
        height: calc(var(--sp-player-progress-height) + 4px);
        --paper-progress-height: calc(var(--sp-player-progress-height) + 4px);
      }
      .sp-progress[paused] paper-progress {
...
customElements.define('sp-progress', SonosPlayerProgress);
